        ...
        {mobileOpen && (
          ...
          >
            <div className="px-6 py-4 flex flex-col gap-4">
              <Link href="/#home" onClick={() => setMobileOpen(false)} className="text-gray-700 font-medium hover:text-primary-600 transition-colors py-1">Home</Link>
              {/* Vending Machines */}
              <div>
                <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold mb-1.5">Vending Machines</p>
                {vendingDropdown.map((item) => (
                  <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className="block text-gray-700 hover:text-primary-600 transition-colors py-1 pl-2 text-sm">{item.label}</Link>
                ))}
                <Link href="/products/sanitary-napkin-vending-machines" onClick={() => setMobileOpen(false)} className="block text-primary-600 font-medium transition-colors py-1 pl-2 text-sm">→ All Vending Machines</Link>
              </div>
              {/* Incinerators */}
              <div>
                <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold mb-1.5">Incinerators</p>
                {incineratorDropdown.map((item) => (
                  <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className="block text-gray-700 hover:text-primary-600 transition-colors py-1 pl-2 text-sm">{item.label}</Link>
                ))}
                <Link href="/products/sanitary-napkin-incinerators" onClick={() => setMobileOpen(false)} className="block text-primary-600 font-medium transition-colors py-1 pl-2 text-sm">→ All Incinerators</Link>
              </div>
              {/* States */}
              <div>
                <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold mb-1.5">States</p>
              ...
              <Link href="/blog" onClick={() => setMobileOpen(false)} className="text-gray-700 font-medium hover:text-primary-600 transition-colors py-1">Blog</Link>
              <Link href="/#contact" onClick={() => setMobileOpen(false)} className="text-gray-700 font-medium hover:text-primary-600 transition-colors py-1">Contact</Link>
              <div className="pt-2 pb-1 border-t border-gray-100">
                <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold mb-3">Quick Contact</p>
                <div className="grid grid-cols-3 gap-2">
                  <a
                    href="tel:+918122378860"
                    className="flex flex-col items-center gap-1.5 py-3 rounded-xl bg-primary-50 border border-primary-100 active:bg-primary-100 transition-colors"
                  >
                  ...
                  <a
                    href={`https://wa.me/918122378860?text=${encodeURIComponent("Hi! I'm interested in Lyra Enterprise's vending machines / incinerators. Please share details.")}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex flex-col items-center gap-1.5 py-3 rounded-xl bg-green-50 border border-green-100 active:bg-green-100 transition-colors"
                  >
                  ...
                  <a
                    ...
                    className="flex flex-col items-center gap-1.5 py-3 rounded-xl bg-indigo-50 border border-indigo-100 active:bg-indigo-100 transition-colors"
                  >
              ...
              <Link
                ...
                className="px-5 py-3 text-sm font-semibold text-white bg-purple-gradient rounded-full text-center shadow-purple"
              >
                Get a Quote
              </Link>
            </div>
